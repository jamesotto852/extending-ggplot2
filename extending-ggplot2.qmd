...
<h1> Extending **ggplot2** </h1>

<h2> with custom `Geom`s and `Stat`s </h2>

<hr>

<h3> James Otto </h3>

![](images/ggplot2.svg){.absolute top=270 left=670 width="300"}
...
* `GeomPoint` and `StatIdentity` are each objects\* with methods and fields that encode the relevant 
...
we can create a `GeomPointNew` from `GeomPoint` with a few adjustments to the default aesthetic options:
...
* [**ggtrace**](https://yjunechoe.github.io/ggtrace/) for interactive debugging of **ggplot2** internals
* [R Graphics](https://www.stat.auckland.ac.nz/~paul/RG2e/) by Paul Murrell

# Thank you! 

[jamesotto852.github.io/extending-ggplot2](https://jamesotto852.github.io/extending-ggplot2)
...
```{r}
#| code-line-numbers: "1-9|11-12|14-16|18-20" 
A <- ggproto("A", NULL,
...
## Prototypes {data-menu-title="Prototypes 1"}
...
<!-- ## Prototypes {data-menu-title="Prototypes 2"} -->
## {data-menu-title="Prototypes 2"}

```{r}
#| code-line-numbers: "1-9|11-13|15-17|19-21|23-25" 
A <- ggproto("A", NULL,
...
## Avoiding **grid** with `$setup_data()` {data-menu-title="`$setup_data()`"}

::: {.incremental}

* The `$setup_data()` method allows `Geom`s to "intercept" the layer's data 
before the `$draw_*()` hierarchy

* This is of limited use, mainly for "row-wise" operations

* We can attempt to implement `GeomComplete` with this strategy, 
however we will quickly run into problems

:::


## {data-menu-title="`$setup_data()` 1"}
...
## Another way to avoid **grid** {data-menu-title="Borrowing `draw_*()` methods"}

::: {.incremental}

* We can use existing `$draw_layer()`, `$draw_panel()`, and `$draw_group()`
methods in new `Geom` objects.

* This allows a much easier (and less error-prone) implementation of `GeomComplete`

:::

## {data-menu-title="Borrowing `draw_*()` methods 1"}

```{r}
#| code-line-numbers: "|14-23|16-18|20-21" 
GeomComplete <- ggproto("GeomComplete", Geom,
...
<!-- indices_keep <- as.vector(upper.tri(diag(nrow(data)))) -->
<!-- data_expanded <- data_expanded[indices_keep, ] -->
    
## {data-menu-title="Borrowing `draw_*()` methods 2"}
...
* Aside from implementing new `Geom` and `Stat` `ggproto` objects,
there are other formal ways to extend **ggplot2**:
...
p4 <- ggplot(penguins, aes(x = bill_length_mm, y = species)) +
  ggbeeswarm::geom_quasirandom() +
  lemon::coord_flex_cart(left = lemon::brackets_vertical("right", length = unit(.12, "npc")))
...
p6 <- ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm, color = species)) +
  geom_point() +
  ggforce::facet_zoom(x = species == "Adelie")

library("geofacet")
p7 <- ggplot(geofacet::state_unemp, aes(year, rate)) +
  geom_line() +
...
## {data-menu-title="Other extension types 1" .smaller}
...
## {data-menu-title="Other extension types 2" .smaller}
...
## {data-menu-title="Other extension types 3" .smaller}
...
## {data-menu-title="Other extension types 4" .smaller}
...
## {data-menu-title="Other extension types 5" .smaller}
...
## {data-menu-title="Other extension types 6" .smaller}
...
## {data-menu-title="Other extension types 7" .smaller}
...
## Package development -- `ggproto` {data-menu-title="Package Development (ggproto)"}

::: {.incremental}

* R packages for **ggplot2** extensions look slightly different
than most other packages[^3]
  ...
  * Example: `ggdensity::GeomHdr`'s [source code](https://github.com/jamesotto852/ggdensity/blob/main/R/hdr.R)

:::


[^3]: ["R Packages"](https://r-pkgs.org/) by Hadley Wickham is a great resource for general package development in R.


## Package development -- documentation {.smaller data-menu-title="Package Development (documentation 1)"}
  
::: {.incremental}

* It is easiest to inherit parameter documentation where possible
  * Almost always inherit from `ggplot2::stat_identity()`
  * Sometimes from relevant `Geom`s (e.g. `ggplot2::geom_segment()`) 
...
## {.smaller data-menu-title="Package Development (documentation 2)"}

```{r}
#| eval: false
#| code-line-numbers: "|25-36|25-27|28-36|7-18|20-23|37-38|107,109|143,148|210,212|235,239" 
...
## Package development -- unit testing {data-menu-title="Package Development (testing)"}
  
::: {.incremental}

* Basic tools from [**testthat**](https://testthat.r-lib.org/) work well for
testing logic in `$compute_*()` methods[^4].

* Visual unit tests are available with [**vdiffr**](https://github.com/r-lib/vdiffr)
  * Allow for programmatic comparison of graphics generated by **ggplot2** 
  * High sensitivity; low precision -- lots of false positives

[^4]: See the [chapter on unit tests](https://r-pkgs.org/testing-basics.html) in "R Packages" for details
